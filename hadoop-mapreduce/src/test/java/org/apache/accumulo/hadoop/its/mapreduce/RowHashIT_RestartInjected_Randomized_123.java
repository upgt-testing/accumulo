...
public class RowHashIT_RestartInjected_Randomized_123 extends ConfigurableMacBase {
    ...
    static void runTest(AccumuloClient c, MiniAccumuloClusterImpl cluster) throws AccumuloException, AccumuloSecurityException, TableExistsException, TableNotFoundException, MutationsRejectedException, IOException, InterruptedException, NoSuchAlgorithmException {
        ...
        bw.close();
        Process hash = cluster.exec(RowHash.class, Collections.singletonList(hadoopTmpDirArg), "-c", cluster.getClientPropsPath(), "-t", tablename, "--column", input_cfcq).getProcess();
    ...
    public static class RowHash extends Configured implements Tool {
        ...
        public int run(String[] args) throws Exception {
            ...
            if (cf.getLength() > 0) {
                AccumuloInputFormat.configure().clientProperties(opts.getClientProps()).table(opts.tableName).auths(Authorizations.EMPTY).fetchColumns(Collections.singleton(new IteratorSetting.Column(cf, cq))).store(job);
            }
