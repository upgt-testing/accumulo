...
/**
 * Health check to verify that expected scan servers are registered (if configured).
 */
public class AccumuloScanServersRegisteredCheck implements HealthCheck<MiniAccumuloCluster> {
  ...
  public HealthCheckResult checkHealth(MiniAccumuloCluster cluster) throws Exception {
    ...
    try {
      int expected = impl.getConfig().getNumScanServers();
      Set<String> registered = client.instanceOperations().getScanServers();

      result.addMetric("expected_scan_servers", expected);
      result.addMetric("registered_scan_servers", registered.size());

      // Only fail if scan servers are expected but not registered
      if (expected > 0 && registered.size() != expected) {
        result.addFailure("Expected " + expected + " scan servers but found " +
            registered.size());
      }
    } finally {
      client.close();
  ...
  public String getName() {
    return "accumulo-scan-servers-registered";
  }
