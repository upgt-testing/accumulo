...
//
public class HalfClosedTabletIT_RestartInjected_Randomized_456 extends SharedMiniClusterBase {
    ...
    public void testSplitWithInvalidContext() throws Exception {
        ...
        try (final var client = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            setInvalidClassLoaderContextPropertyWithoutValidation(getCluster().getServerContext(), tableId);
            ...
            RestartFramework.at("after_split").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testIteratorThrowingTransientError() throws Exception {
        ...
        try (AccumuloClient c = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            RestartFramework.at("after_table_create_test2").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_iterator_attach_test2").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            c.tableOperations().compact(tableName, new CompactionConfig().setWait(true).setFlush(true));
            RestartFramework.at("after_compact_test2").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testBadIteratorOnStack() throws Exception {
        ...
        try (AccumuloClient c = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            tops.create(tableName);
            final var tid = TableId.of(tops.tableIdMap().get(tableName));
            RestartFramework.at("after_table_create_test3").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_bad_iterator_attach_test3").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            try (BatchWriter bw = c.createBatchWriter(tableName)) {
                Mutation m = new Mutation(new Text("r1"));
                m.put("acf", tableName, "1");
                bw.addMutation(m);
            ...
            // minc should fail, so there should be no files
            FunctionalTestUtils.checkRFiles(c, tableName, 1, 1, 0, 0);
            RestartFramework.at("after_failed_offline_test3").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_iterator_remove_test3").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // Minc should have completed successfully
            Wait.waitFor(() -> tabletHasExpectedRFiles(c, tableName, 1, 1, 1, 1), 340_000);
    ...
    public static void setInvalidClassLoaderContextPropertyWithoutValidation(ServerContext context, TableId tableId) {
        ...
        context.getPropStore().putAll(key, Map.of(Property.TABLE_CLASSLOADER_CONTEXT.getKey(), "invalid"));
    }
    ...
    public static long countHostedTablets(AccumuloClient c, TableId tid) {
        try (TabletsMetadata tm = ((ClientContext) c).getAmple().readTablets().forTable(tid).fetch(ColumnType.LOCATION).build()) {
