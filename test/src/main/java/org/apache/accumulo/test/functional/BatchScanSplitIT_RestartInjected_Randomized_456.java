...
public class BatchScanSplitIT_RestartInjected_Randomized_456 extends AccumuloClusterHarness {

    private static final Logger log = LoggerFactory.getLogger(BatchScanSplitIT_RestartInjected.class);
    ...
    public void test() throws Exception {
        try (AccumuloClient c = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            RestartFramework.at("after_splits_occur").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            for (int i = 0; i < 20; i++) {
                ...
                if (i == 10) {
                    RestartFramework.at("during_batch_scan").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                }
            }
            splits = c.tableOperations().listSplits(tableName);
