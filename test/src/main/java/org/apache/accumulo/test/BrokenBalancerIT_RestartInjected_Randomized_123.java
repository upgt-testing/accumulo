...
public class BrokenBalancerIT_RestartInjected_Randomized_123 extends ConfigurableMacBase {

    private static final Logger log = LoggerFactory.getLogger(BrokenBalancerIT_RestartInjected.class);
    ...
    public void testBalancerException() throws Exception {
        ...
        testBadBalancer(BrokenBalancer.class.getName(), tableName, "exception");
    }
    ...
    private void testBadBalancer(String balancerClass, String tableName, String testVariant) throws Exception {
        try (AccumuloClient c = Accumulo.newClient().from(getClientProperties()).build()) {
            ...
            NewTableConfiguration ntc = new NewTableConfiguration().withSplits(splits).setProperties(props);
            c.tableOperations().create(tableName, ntc);
            RestartFramework.at("after_table_create_" + testVariant).on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_fix_table_balancer_" + testVariant).on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_break_manager_balancer_" + testVariant).on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            Wait.waitFor(() -> c.instanceOperations().getSystemConfiguration().get(Property.MANAGER_TABLET_BALANCER.getKey()).equals(balancerClass));
            ...
            RestartFramework.at("after_fix_manager_balancer_" + testVariant).on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
