 ...
 */
@Tag(SUNNY_DAY)
public class BulkIT_RestartInjected_Randomized_456 extends AccumuloClusterHarness {
    ...
    public void test() throws Exception {
        try (AccumuloClient client = Accumulo.newClient().from(getClientProps()).build()) {
            RestartFramework.at("before_bulk_import_test").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            runTest(client, getCluster().getFileSystem(), getCluster().getTemporaryPath(), getUniqueNames(1)[0], this.getClass().getName(), testName(), false);
            RestartFramework.at("after_bulk_import_test").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void testOld() throws Exception {
        try (AccumuloClient client = Accumulo.newClient().from(getClientProps()).build()) {
            RestartFramework.at("before_old_bulk_import_test").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            runTest(client, getCluster().getFileSystem(), getCluster().getTemporaryPath(), getUniqueNames(1)[0], this.getClass().getName(), testName(), true);
            RestartFramework.at("after_old_bulk_import_test").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }

    static void runTest(AccumuloClient c, FileSystem fs, Path basePath, String tableName, String filePrefix, String dirSuffix, boolean useOld) throws Exception {
    ...
    private static void bulkLoad(AccumuloClient c, String tableName, Path bulkFailures, Path files, boolean useOld) throws TableNotFoundException, IOException, AccumuloException, AccumuloSecurityException {
        ...
        if (useOld) {
            ...
            try {
                ...
                c.tableOperations().importDirectory(files.toString()).to(tableName).ignoreEmptyDir(false).load();
            } catch (IllegalArgumentException ex) {
                // expected exception to be thrown
            }
        }
