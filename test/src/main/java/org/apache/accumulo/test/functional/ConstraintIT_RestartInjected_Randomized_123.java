...
public class ConstraintIT_RestartInjected_Randomized_123 extends AccumuloClusterHarness {

    private static final Logger log = LoggerFactory.getLogger(ConstraintIT_RestartInjected.class);
    ...
    public void run() throws Exception {
        ...
        try (AccumuloClient c = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            RestartFramework.at("after_table_creation_and_constraint_setup").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_constraint_verification").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            log.debug("Verified constraints on all tables. Running tests");
            test1(c, tableNames[0]);
            RestartFramework.at("after_test1_complete").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            test2(c, tableNames[1], false);
            RestartFramework.at("after_test2_no_flush").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            test2(c, tableNames[2], true);
            RestartFramework.at("after_test2_with_flush").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    private void test2(AccumuloClient client, String table, boolean doFlush) throws Exception {
        ...
        try (Scanner scanner = client.createScanner(table, Authorizations.EMPTY)) {
            ...
            entry = iter.next();
            if (!entry.getKey().getRow().equals(new Text("r1")) || !entry.getKey().getColumnFamily().equals(new Text("cf1")) || !entry.getKey().getColumnQualifier().equals(new Text("cq4")) || !entry.getValue().equals(new Value("789"))) {
                throw new Exception("Unexpected key or value " + entry.getKey() + " " + entry.getValue());
            }
            if (iter.hasNext()) {
                entry = iter.next();
