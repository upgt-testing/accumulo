...
public class AddSplitIT_RestartInjected_Randomized_456 extends AccumuloClusterHarness {
    ...
    public void addSplitTest() throws Exception {
        ...
        try (AccumuloClient c = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            RestartFramework.at("after_first_data_insert").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            TreeSet<Text> splits = new TreeSet<>();
            splits.add(new Text(String.format("%09d", 333)));
            splits.add(new Text(String.format("%09d", 666)));
            c.tableOperations().addSplits(tableName, splits);
            RestartFramework.at("after_first_add_splits").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_first_verify_data").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            insertData(c, tableName, 2L);
            RestartFramework.at("after_second_data_insert").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            c.tableOperations().addSplits(tableName, splits);
            RestartFramework.at("after_second_add_splits").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            sleepUninterruptibly(100, TimeUnit.MILLISECONDS);
