...
public class BinaryStressIT_RestartInjected_Randomized_456 extends AccumuloClusterHarness {
    ...
    public void binaryStressTest() throws Exception {
        try (AccumuloClient c = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            RestartFramework.at("after_binary_test_run").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            String id = c.tableOperations().tableIdMap().get(tableName);
            RestartFramework.at("after_table_id_retrieved").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            try (Scanner s = c.createScanner(MetadataTable.NAME, Authorizations.EMPTY)) {
                ...
                for (Entry<Key, Value> entry : s) {
                    ...
                    scannedCount++;
                    if (scannedCount == 4) {
                        RestartFramework.at("during_metadata_scan").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                    }
                }
            }
