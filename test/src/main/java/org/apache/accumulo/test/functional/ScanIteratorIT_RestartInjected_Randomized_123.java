...
@Tag(SUNNY_DAY)
public class ScanIteratorIT_RestartInjected_Randomized_123 extends AccumuloClusterHarness {

    private static final Logger log = LoggerFactory.getLogger(ScanIteratorIT_RestartInjected.class);
    ...
    public void setup() throws Exception {
        ...
        RestartFramework.at("after_change_authorizations").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_grant_read_permission").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_grant_write_permission").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        accumuloClient.securityOperations().grantTablePermission(user, tableName, TablePermission.WRITE);
    ...
    public void run() throws Exception {
        ...
        try (AccumuloClient c = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            try (Scanner scanner = c.createScanner(tableName);
                ...
                setupIter(scanner);
                RestartFramework.at("after_scanner_iterator_setup").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                verify(scanner, 1, 999);
                RestartFramework.at("after_scanner_verify").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                bscanner.setRanges(Collections.singleton(new Range((Key) null, null)));
                setupIter(bscanner);
                RestartFramework.at("after_batchscanner_iterator_setup").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                verify(bscanner, 1, 999);
                RestartFramework.at("after_batchscanner_verify_full_range").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                RestartFramework.at("after_batchscanner_multi_range_setup").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void runTest(Authorizations auths, boolean shouldFail) throws Exception {
        ClusterUser clusterUser = getUser(0);
        AccumuloClient userC = getCluster().createAccumuloClient(clusterUser.getPrincipal(), clusterUser.getToken());
        writeTestMutation(userC);
        RestartFramework.at("after_write_test_mutation").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        try (Scanner scanner = userC.createScanner(tableName, auths);
            ...
            RestartFramework.at("after_scanner_iterator_add").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            batchScanner.setRanges(Collections.singleton(new Range("1")));
            batchScanner.addScanIterator(setting);
            RestartFramework.at("after_batchscanner_iterator_add").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
