...
public class LargeRowIT_RestartInjected_Randomized_456 extends AccumuloClusterHarness {

    private static final Logger log = LoggerFactory.getLogger(LargeRowIT_RestartInjected.class);
    ...
    public void run() throws Exception {
        ...
        try (AccumuloClient c = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            RestartFramework.at("after_create_regular_table").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            c.tableOperations().create(PRE_SPLIT_TABLE_NAME, new NewTableConfiguration().setProperties(singletonMap(Property.TABLE_MAX_END_ROW_SIZE.getKey(), "256K")).withSplits(splitPoints));
            RestartFramework.at("after_create_presplit_table").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void test1(AccumuloClient c) throws Exception {
        ...
        RestartFramework.at("after_split_check").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        log.info("checking splits");
        RestartFramework.at("after_set_split_threshold").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        FunctionalTestUtils.checkSplits(c, REG_TABLE_NAME, NUM_PRE_SPLITS / 2, NUM_PRE_SPLITS * 4);
        RestartFramework.at("after_basic_test_reg").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private void basicTest(AccumuloClient c, String table, int expectedSplits) throws Exception {
        ...
        RestartFramework.at("after_write_large_rows").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        FunctionalTestUtils.checkSplits(c, table, expectedSplits, expectedSplits);
        verify(c, table);
        RestartFramework.at("after_flush_blocking").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        FunctionalTestUtils.checkSplits(c, table, expectedSplits, expectedSplits);
        c.tableOperations().flush(table, null, null, false);
        RestartFramework.at("after_final_verify").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // verify while table flush is running
        verify(c, table);
        // give split time to complete
        c.tableOperations().flush(table, null, null, true);
        RestartFramework.at("after_flush_nonblocking").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        FunctionalTestUtils.checkSplits(c, table, expectedSplits, expectedSplits);
        verify(c, table);
        FunctionalTestUtils.checkSplits(c, table, expectedSplits, expectedSplits);
    }
