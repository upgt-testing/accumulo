 ...
 */
public class TabletMetadataIT_RestartInjected_Randomized_456 extends ConfigurableMacBase {

    private static final Logger log = LoggerFactory.getLogger(TabletMetadataIT_RestartInjected.class);
    ...
    public void getLiveTServersTest() throws Exception {
        try (AccumuloClient c = Accumulo.newClient().from(getClientProperties()).build()) {
            ...
            RestartFramework.at("after_initial_tserver_count").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            RestartFramework.at("before_kill_tserver").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_tserver_killed").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            servers = TabletMetadata.getLiveTServers((ClientContext) c);
            assertEquals(NUM_TSERVERS - 1, servers.size());
            RestartFramework.at("after_final_tserver_count").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
}
