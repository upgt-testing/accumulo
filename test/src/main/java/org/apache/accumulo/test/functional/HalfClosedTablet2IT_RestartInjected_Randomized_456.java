...
public class HalfClosedTablet2IT_RestartInjected_Randomized_456 extends SharedMiniClusterBase {
    ...
    public void testInvalidContextCausesVolumeChooserFailure() throws Exception {
        ...
        try (final var client = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            HalfClosedTabletIT.setInvalidClassLoaderContextPropertyWithoutValidation(getCluster().getServerContext(), tableId);
            // Need to wait for TabletServer to pickup configuration change
            Thread.sleep(3000);
            RestartFramework.at("after_set_invalid_context").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_flush_with_invalid_context").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            HalfClosedTabletIT.removeInvalidClassLoaderContextProperty(client, tableName);
            // Minc should have completed successfully
            Wait.waitFor(() -> HalfClosedTabletIT.tabletHasExpectedRFiles(client, tableName, 1, 1, 1, 1), 340_000);
            RestartFramework.at("after_minc_completion").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
