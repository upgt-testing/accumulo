 ...
 */
public class CountNameNodeOpsBulkIT_RestartInjected_Randomized_123 extends ConfigurableMacBase {
    ...
    public void compareOldNewBulkImportTest() throws Exception {
        try (AccumuloClient c = Accumulo.newClient().from(getClientProperties()).build()) {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                futures.add(es.submit(() -> {
                    ...
                    for (int i1 = 0; i1 < 100; i1++) {
                        FileSKVWriter writer = FileOperations.getInstance().newWriterBuilder().forFile(files + "/bulk_" + i1 + "." + RFile.EXTENSION, fs, fs.getConf(), NoCryptoServiceFactory.NONE).withTableConfiguration(DefaultConfiguration.getInstance()).build();
            ...
            RestartFramework.at("after_bulk_files_created").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            log.info("Importing");
