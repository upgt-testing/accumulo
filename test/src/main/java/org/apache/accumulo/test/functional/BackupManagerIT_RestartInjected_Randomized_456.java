...
public class BackupManagerIT_RestartInjected_Randomized_456 extends ConfigurableMacBase {
    ...
    public void test() throws Exception {
        ...
        try (AccumuloClient client = Accumulo.newClient().from(getClientProperties()).build()) {
            ...
            UtilWaitThread.sleep(1000);
            RestartFramework.at("after_backup_manager_established").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_zk_event_generated").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // kill the manager by removing its lock
            writer.recursiveDelete(lockPath, NodeMissingPolicy.FAIL);
            RestartFramework.at("after_manager_lock_removed").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // ensure the backup becomes the manager
            client.tableOperations().create(getUniqueNames(1)[0]);
            RestartFramework.at("after_backup_promotion_verified").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        } finally {
            backup.destroy();
        }
    }
