 ...
 */
public class TransportCachingIT_RestartInjected_Randomized_123 extends AccumuloClusterHarness {
    ...
    public void testCachedTransport() throws InterruptedException {
        try (AccumuloClient client = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            RestartFramework.at("after_tservers_up").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            List<ThriftTransportKey> servers = tservers.stream().map(serverStr -> {
                return new ThriftTransportKey(ThriftClientTypes.CLIENT, HostAndPort.fromString(serverStr), rpcTimeout, context);
            ...
            RestartFramework.at("after_first_transport").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_cache_test").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_noncache_test").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_lifo_test").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    private TTransport getAnyTransport(ThriftTransportKey ttk, ThriftTransportPool pool, boolean preferCached) {
        ...
        try {
            return pool.getTransport(ttk.getType(), ttk.getServer(), ttk.getTimeout(), getServerContext(), preferCached);
