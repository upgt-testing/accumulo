...
public class VisibilityIT_RestartInjected_Randomized_456 extends AccumuloClusterHarness {
    ...
    public void run() throws Exception {
        try (AccumuloClient c = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            insertData(c, table);
            RestartFramework.at("after_insert_data").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            queryData(c, table);
            RestartFramework.at("after_query_data").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            deleteData(c, table);
            RestartFramework.at("after_delete_data").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            insertDefaultData(c, table2);
            RestartFramework.at("after_insert_default_data").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
