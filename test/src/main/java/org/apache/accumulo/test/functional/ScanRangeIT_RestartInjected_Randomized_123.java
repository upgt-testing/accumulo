...
public class ScanRangeIT_RestartInjected_Randomized_123 extends AccumuloClusterHarness {
    ...
    public void run() throws Exception {
        try (AccumuloClient c = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            RestartFramework.at("after_table2_create_with_splits").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            insertData(c, table1);
            RestartFramework.at("after_insertData_table1").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            scanTable(c, table1);
            RestartFramework.at("after_scanTable_table1").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            insertData(c, table2);
            RestartFramework.at("after_insertData_table2").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            scanTable(c, table2);
            RestartFramework.at("after_scanTable_table2").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    private void scanRange(AccumuloClient c, String table, IntKey ik1, boolean inclusive1, IntKey ik2, boolean inclusive2) throws Exception {
                ...
                for (int j = 0; j < CF_LIMIT; j++) {
                    for (int k = 0; k < CQ_LIMIT; k++) {
                        for (int t = 0; t < TS_LIMIT; t++) {
                            m.put(createCF(j), createCQ(k), t, new Value(String.format("%06d_%03d_%03d_%03d", i, j, k, t)));
                        }
                    }
                }
                bw.addMutation(m);
