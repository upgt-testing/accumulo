...
public class BadCompactionServiceConfigIT_RestartInjected_Randomized_456 extends AccumuloClusterHarness {
    ...
    public void testUsingMisconfiguredService() throws Exception {
        ...
        try (AccumuloClient client = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            RestartFramework.at("after_table_create_misconfig").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            try (var writer = client.createBatchWriter(table)) {
                writer.addMutation(new Mutation("0").at().family("f").qualifier("q").put("v"));
            }
            RestartFramework.at("after_batch_write_misconfig").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            client.tableOperations().flush(table, null, null, true);
            RestartFramework.at("after_flush_misconfig").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            try (var scanner = client.createScanner(table)) {
                assertEquals("0", scanner.stream().map(e -> e.getKey().getRowData().toString()).collect(MoreCollectors.onlyElement()));
            }
            Future<?> fixerFuture = executorService.submit(() -> {
                try {
                    ...
                    var value = "[{'name':'small', 'type': 'internal', 'numThreads':1}]".replaceAll("'", "\"");
            ...
            fixerFuture.get();
            RestartFramework.at("after_fix_config_misconfig").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testUsingNonExistentService() throws Exception {
        ...
        try (AccumuloClient client = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            client.tableOperations().create(table, ntc);
            RestartFramework.at("after_table_create_nonexistent").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            var splits = IntStream.range(1, 10).mapToObj(i -> new Text(i + "")).collect(Collectors.toCollection(TreeSet::new));
            client.tableOperations().addSplits(table, splits);
            RestartFramework.at("after_add_splits_nonexistent").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            try (var writer = client.createBatchWriter(table)) {
                writer.addMutation(new Mutation("0").at().family("f").qualifier("q").put("v"));
            }
            RestartFramework.at("after_batch_write_nonexistent").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            client.tableOperations().flush(table, null, null, true);
            RestartFramework.at("after_flush_nonexistent").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            try (var scanner = client.createScanner(table)) {
                assertEquals("0", scanner.stream().map(e -> e.getKey().getRowData().toString()).collect(MoreCollectors.onlyElement()));
            }
            ...
            Future<?> fixerFuture = executorService.submit(() -> {
                try {
                    ...
                    client.tableOperations().setProperty(table, Property.TABLE_COMPACTION_DISPATCHER_OPTS.getKey() + "service", DEFAULT_COMPACTION_SERVICE_NAME);
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
            });
            ...
            fixerFuture.get();
            RestartFramework.at("after_fix_config_nonexistent").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
}
