...
public class ScanSessionTimeOutIT_RestartInjected_Randomized_123 extends AccumuloClusterHarness {
    ...
    public void reduceSessionIdle() throws Exception {
        try (AccumuloClient client = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            RestartFramework.at("after_session_idle_property_set").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_session_idle_wait").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void resetSessionIdle() throws Exception {
        if (sessionIdle != null) {
            try (AccumuloClient client = Accumulo.newClient().from(getClientProps()).build()) {
                client.instanceOperations().setProperty(Property.TSERV_SESSION_MAXIDLE.getKey(), sessionIdle);
            }
        }
    }
    ...
    public void run() throws Exception {
        try (AccumuloClient c = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            try (Scanner scanner = c.createScanner(tableName, new Authorizations())) {
                ...
                RestartFramework.at("after_first_verify_with_active_session").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                RestartFramework.at("after_session_timeout_verification").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                verify(iter, 200, 50000);
                // Reading part of the data in the range should cause a new scan session to be created
                assertEquals(1L, countActiveScans(c, TABLET_SERVER, tableName));
                RestartFramework.at("after_second_verify_new_session").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                verify(iter, 50000, 100000);
                // Once all of the data in the range was read the scanner should automatically close the
                // scan session
                assertEquals(0L, countActiveScans(c, TABLET_SERVER, tableName));
                RestartFramework.at("after_third_verify_session_closed").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
            // Nothing should have created any ew scan sessions for the table
            assertEquals(0L, countActiveScans(c, TABLET_SERVER, tableName));
            RestartFramework.at("after_final_scan_verification").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
