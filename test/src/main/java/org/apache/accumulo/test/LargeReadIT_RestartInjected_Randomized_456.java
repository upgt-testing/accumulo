 ...
 */
public class LargeReadIT_RestartInjected_Randomized_456 extends AccumuloClusterHarness {
    ...
    public void testLargeMemoryLG() throws Exception {
        ...
        Map<String, Set<Text>> groups = Map.of("big", Set.of(new Text("big")), "small", Set.of(new Text("small")));
    ...
    private void testLargeMemory(NewTableConfiguration config, Consumer<Scanner> scannerConfigurer) throws Exception {
        try (AccumuloClient client = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            // to timeout.
            var tasks = Stream.iterate(scanTask, t -> t).limit(numThreads * 5).collect(Collectors.toList());
            ...
            try (var tablets = ctx.getAmple().readTablets().forTable(ctx.getTableId(tableName)).fetch(TabletMetadata.ColumnType.FILES).build()) {
