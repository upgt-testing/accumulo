 ...
 */
public class BalanceInPresenceOfOfflineTableIT_RestartInjected_Randomized_456 extends AccumuloClusterHarness {

    private static final Logger log = LoggerFactory.getLogger(BalanceInPresenceOfOfflineTableIT_RestartInjected.class);
    ...
    public void setupTables() throws AccumuloException, AccumuloSecurityException, TableExistsException, TableNotFoundException {
        ...
        assumeTrue(accumuloClient.instanceOperations().getTabletServers().size() >= 2, "Not enough tservers to run test");
        // set up splits
        ...
        RestartFramework.at("after_unused_table_splits").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // mark the table offline before it can rebalance.
        accumuloClient.tableOperations().offline(UNUSED_TABLE);
        RestartFramework.at("after_test_table_property_set").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // actual test table
        ...
        RestartFramework.at("after_unused_table_offline").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        accumuloClient.tableOperations().setProperty(TEST_TABLE, Property.TABLE_SPLIT_THRESHOLD.getKey(), "10K");
    }
    ...
    public void test() throws Exception {
        RestartFramework.at("after_test_ingest").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        while (!balancingWorked && (System.currentTimeMillis() - startTime) < ((5 * 60 + 15) * 1000)) {
            ...
            ManagerMonitorInfo stats = ThriftClientTypes.MANAGER.execute((ClientContext) accumuloClient, client -> client.getManagerStats(TraceUtil.traceInfo(), creds.toThrift(accumuloClient.instanceOperations().getInstanceId())));
