...
// ACCUMULO-3030
public class InterruptibleScannersIT_RestartInjected_Randomized_123 extends AccumuloClusterHarness {
            ...
            try (Scanner scanner = client.createScanner(tableName, Authorizations.EMPTY)) {
                ...
                Thread thread = new Thread(() -> {
                    try {
                        ...
                        do {
                            ArrayList<ActiveScan> scans = new ArrayList<>(client.instanceOperations().getActiveScans(tserver));
                            // Remove scans not against our table and not owned by us
                            scans.removeIf(scan -> !getAdminPrincipal().equals(scan.getUser()) || !tableName.equals(scan.getTable()));
                            if (!scans.isEmpty()) {
                                // We found our scan
                                break;
                            }
                ...
                try {
                    ...
                    assertThrows(RuntimeException.class, () -> Iterators.size(scanner.iterator()), "Scan should not succeed");
                } finally {
                    thread.join();
                }
            }
