...
public class BloomFilterIT_RestartInjected_Randomized_123 extends AccumuloClusterHarness {

    private static final Logger log = LoggerFactory.getLogger(BloomFilterIT_RestartInjected.class);
    ...
    public void test() throws Exception {
        try (AccumuloClient c = Accumulo.newClient().from(getClientProps()).build()) {
            final String readAhead = c.instanceOperations().getSystemConfiguration().get(Property.TSERV_SCAN_EXECUTORS_DEFAULT_THREADS.getKey());
            c.instanceOperations().setProperty(Property.TSERV_SCAN_EXECUTORS_DEFAULT_THREADS.getKey(), "1");
            RestartFramework.at("after_scan_executors_property_set").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            try {
                Thread.sleep(1000);
                ...
                RestartFramework.at("after_all_tables_created_configured").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                log.info("Writing");
                write(c, tables[0], 1, 0, 2000000000, 500);
                write(c, tables[1], 2, 0, 2000000000, 500);
                write(c, tables[2], 3, 0, 2000000000, 500);
                log.info("Writing complete");
                RestartFramework.at("after_all_writes_complete").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                RestartFramework.at("after_empty_key_flush").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                for (String table : Arrays.asList(tables[0], tables[1], tables[2])) {
                    c.tableOperations().compact(table, null, null, true, true);
                }
                RestartFramework.at("after_compaction_without_bloom").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                // ensure compactions are finished
                for (String table : tables) {
                    FunctionalTestUtils.checkRFiles(c, table, 1, 1, 1, 1);
                }
                RestartFramework.at("after_rfiles_check").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                RestartFramework.at("after_base_query").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                RestartFramework.at("after_bloom_properties_set").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                RestartFramework.at("after_compaction_with_bloom").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                RestartFramework.at("after_bloom_query").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                ...
                RestartFramework.at("after_empty_key_scan").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            } finally {
                c.instanceOperations().setProperty(Property.TSERV_SCAN_EXECUTORS_DEFAULT_THREADS.getKey(), readAhead);
            }
        }
    }
    ...
    private long query(AccumuloClient c, String table, int depth, long start, long end, int num, int step) throws Exception {
    ...
    private void write(AccumuloClient c, String table, int depth, long start, long end, int step) throws Exception {
