...
public class ConcurrencyIT_RestartInjected_Randomized_456 extends AccumuloClusterHarness {
    ...
    public void run() throws Exception {
        try (AccumuloClient c = Accumulo.newClient().from(getClientProps()).build()) {
            runTest(c, getUniqueNames(1)[0], getCluster());
        }
    }

    static void runTest(AccumuloClient c, String tableName, Object cluster) throws Exception {
        RestartFramework.at("after_iterator_attach_concurrency").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_table_create_concurrency").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        RestartFramework.at("after_all_scans_complete").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        bw.close();
    }
}
