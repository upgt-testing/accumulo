...
public class TimeoutIT_RestartInjected_Randomized_456 extends AccumuloClusterHarness {
    ...
    public void testBatchWriterTimeout(AccumuloClient client, String tableName) throws Exception {
        ...
        RestartFramework.at("after_mutation_added").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        mut.put("cf1", "cq1", "v1");
        bw.addMutation(mut);
        var mre = assertThrows(MutationsRejectedException.class, bw::close, "batch writer did not timeout");
        if (mre.getCause() instanceof TimedOutException) {
            return;
        }
        RestartFramework.at("after_table_create_for_bw_timeout").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        RestartFramework.at("after_constraint_added").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        throw mre;
    }

    public void testBatchScannerTimeout(AccumuloClient client, String tableName) throws Exception {
        client.tableOperations().create(tableName);
        RestartFramework.at("after_table_create_for_bs_timeout").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        ...
        try (BatchScanner bs = client.createBatchScanner(tableName)) {
            ...
            RestartFramework.at("after_iterator_added").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
