...
public class ManyWriteAheadLogsIT_RestartInjected_Randomized_456 extends AccumuloClusterHarness {
    ...
    public void testMany() throws Exception {
        ...
        try (AccumuloClient c = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            RestartFramework.at("after_many_wals_table_create").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            c.tableOperations().create(rollWALsTable);
            RestartFramework.at("after_roll_wals_table_create").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            Set<String> allWalsSeen = new HashSet<>();
            addOpenWals(context, allWalsSeen);
            RestartFramework.at("after_initial_wal_tracking").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            try (BatchWriter manyWALsWriter = c.createBatchWriter(manyWALsTable);
                ...
                for (int i = 0; i < 100; i++) {
                    ...
                    addOpenWals(context, allWalsSeen);
                    if (i == 25) {
                        RestartFramework.at("at_25_percent_write_progress").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                    }
                    if (i == 50) {
                        RestartFramework.at("at_50_percent_write_progress").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                    }
                    if (i == 75) {
                        RestartFramework.at("at_75_percent_write_progress").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                    }
                }
            }
            RestartFramework.at("after_write_loop_complete").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertTrue(allWalsSeen.size() >= 50, "Number of WALs seen was less than expected " + allWalsSeen.size());
            RestartFramework.at("after_wal_count_verification").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // the total number of closed write ahead logs should get small
            int closedLogs = countClosedWals(context);
            RestartFramework.at("before_wal_shrinkage_wait").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            while (closedLogs > 3) {
