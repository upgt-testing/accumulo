...
// ACCUMULO-4643
public class YieldScannersIT_RestartInjected_Randomized_123 extends AccumuloClusterHarness {

    Logger log = LoggerFactory.getLogger(YieldScannersIT_RestartInjected.class);
    ...
    public void testScan() throws Exception {
        ...
        try (AccumuloClient client = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            writer.flush();
            writer.close();
            RestartFramework.at("after_batch_write_flush").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            try (Scanner scanner = client.createScanner(tableName, Authorizations.EMPTY)) {
                ...
                RestartFramework.at("after_iterator_setup").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                log.info("iterating");
                ...
                while (it.hasNext()) {
                    ...
                    assertEquals(Character.toString(expected), next.getKey().getRow().toString(), "Unexpected row");
                    ...
                    assertEquals(Integer.toString(yieldNextCount + yieldSeekCount), value[2], "Unexpected rebuild count");
                    keyCount++;
                }
                assertEquals(10, keyCount, "Did not get the expected number of results");
                RestartFramework.at("after_scan_completion").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            }
        }
    }
        ...
        try (AccumuloClient client = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            try (BatchScanner scanner = client.createBatchScanner(tableName, Authorizations.EMPTY, 1)) {
                ...
                scanner.stream().sorted(Map.Entry.comparingByKey()).forEach(entry -> {
                    ...
                    assertEquals(Character.toString(expected), entry.getKey().getRow().toString(), "Unexpected row");
