...
public class AccumuloConfigurationIT_RestartInjected_Randomized_456 extends SharedMiniClusterBase {
    ...
    public void testInvalidation() throws Exception {
        ...
        RestartFramework.at("after_cache_invalidation").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        while (ctx.getConfiguration().get(fakeProperty).equals(initialThreads)) {
            oldValueReturned++;
            Thread.sleep(25);
        }
        System.out.println("Configuration returned old value " + oldValueReturned + " times and took " + timer.elapsed(TimeUnit.MILLISECONDS) + "ms");
