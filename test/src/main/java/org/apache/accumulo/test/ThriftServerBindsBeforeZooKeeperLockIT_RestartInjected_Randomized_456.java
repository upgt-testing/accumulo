...
public class ThriftServerBindsBeforeZooKeeperLockIT_RestartInjected_Randomized_456 extends AccumuloClusterHarness {

    private static final Logger LOG = LoggerFactory.getLogger(ThriftServerBindsBeforeZooKeeperLockIT_RestartInjected.class);
    ...
    public void testMonitorService() throws Exception {
        ...
        RestartFramework.at("after_monitor_start").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        String monitorUrl = "http://localhost:" + freePort;
        Process monitor = null;
        RestartFramework.at("after_active_monitor_found").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        try {
            LOG.debug("Starting standby monitor on {}", freePort);
            monitor = startProcess(cluster, ServerType.MONITOR, freePort);
            RestartFramework.at("after_standby_monitor_start").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            while (true) {
                ...
                try {
                    ...
                    if (responseCode == HttpURLConnection.HTTP_OK) {
                        RestartFramework.at("after_monitor_http_success").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                        return;
                    } else {
    ...
    public void testManagerService() throws Exception {
        ...
        try (AccumuloClient client = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            while (true) {
                try {
                    List<String> locks = cluster.getServerContext().getZooReader().getChildren(Constants.ZROOT + "/" + instanceID + Constants.ZMANAGER_LOCK);
                    if (!locks.isEmpty()) {
                        break;
                    }
                } catch (Exception e) {
            ...
            RestartFramework.at("after_active_manager_found").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            int freePort = PortUtils.getRandomFreePort();
            ...
            try {
                ...
                RestartFramework.at("after_standby_manager_start").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                while (true) {
                    try (Socket s = new Socket("localhost", freePort)) {
                        if (s.isConnected()) {
                            RestartFramework.at("after_manager_socket_success").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                            // Pass
                            return;
                        }
                    } catch (Exception e) {
    ...
    public void testGarbageCollectorPorts() throws Exception {
        ...
        try (AccumuloClient client = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            while (true) {
                try {
                    List<String> locks = cluster.getServerContext().getZooReader().getChildren(Constants.ZROOT + "/" + instanceID + Constants.ZGC_LOCK);
                    if (!locks.isEmpty()) {
                        break;
                    }
                } catch (Exception e) {
            ...
            RestartFramework.at("after_active_gc_found").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            int freePort = PortUtils.getRandomFreePort();
            Process manager = null;
            try {
                LOG.debug("Starting standby gc on {}", freePort);
                manager = startProcess(cluster, ServerType.GARBAGE_COLLECTOR, freePort);
                RestartFramework.at("after_standby_gc_start").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                while (true) {
                    try (Socket s = new Socket("localhost", freePort)) {
                        if (s.isConnected()) {
                            RestartFramework.at("after_gc_socket_success").on(getCluster()).restart("garbage_collector").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
                            // Pass
                            return;
                        }
                    } catch (Exception e) {
    ...
    private Process startProcess(MiniAccumuloClusterImpl cluster, ServerType serverType, int port) throws IOException {
        ...
        return cluster._exec(service, serverType, Map.of(property.getKey(), Integer.toString(port))).getProcess();
    }
}
