...
public class CompactionConfigChangeIT_RestartInjected_Randomized_456 extends AccumuloClusterHarness {
    ...
    public static long countFiles(AccumuloClient client, String table, String fileNamePrefix) throws Exception {
        ...
        try (var tablets = ctx.getAmple().readTablets().forTable(tableId).build()) {
            return tablets.stream().flatMap(tm -> tm.getFiles().stream()).filter(stf -> stf.getFileName().startsWith(fileNamePrefix)).count();
        }
    }
    ...
    public void testRemovingCompactionExecutor() throws Exception {
        ...
        try (AccumuloClient client = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            RestartFramework.at("after_property_setup").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_compact_start").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_config_change").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
