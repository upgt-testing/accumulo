...
public class ExternalCompactionMetricsIT_RestartInjected_Randomized_123 extends SharedMiniClusterBase {
    ...
    public void testMetrics() throws Exception {
        ...
        try (final AccumuloClient client = Accumulo.newClient().from(getCluster().getClientProperties()).build()) {
            ...
            RestartFramework.at("after_table1_create_metrics").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            String table2 = names[1];
            createTable(client, table2, "cs2", 10);
            RestartFramework.at("after_table2_create_metrics").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            writeData(client, table1);
            writeData(client, table2);
            RestartFramework.at("after_data_write_metrics").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_compact_initiate_metrics").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_compactors_start_metrics").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_verify_metrics").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
