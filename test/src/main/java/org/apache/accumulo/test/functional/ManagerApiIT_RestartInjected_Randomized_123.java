...
public class ManagerApiIT_RestartInjected_Randomized_123 extends SharedMiniClusterBase {
    ...
    public static void setup() throws Exception {
        ...
        try (AccumuloClient client = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            RestartFramework.at("after_user_creation").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        }
    }
    ...
    public void testPermissions_initiateFlush() throws Exception {
        ...
        try (AccumuloClient client = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            RestartFramework.at("after_table_create_initiate_flush").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_grant_permissions_initiate_flush").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    public void testPermissions_waitForFlush() throws Exception {
        ...
        String[] uniqNames = getUniqueNames(3);
        String tableName = uniqNames[0];
        RestartFramework.at("after_initiate_flush_wait_flush").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        Credentials regUserWithWrite = new Credentials(uniqNames[1], new PasswordToken(uniqNames[1]));
        Credentials regUserWithAlter = new Credentials(uniqNames[2], new PasswordToken(uniqNames[2]));
        String tableId;
        try (AccumuloClient client = Accumulo.newClient().from(getClientProps()).build()) {
            ...
            rootSecOps.createLocalUser(regUserWithAlter.getPrincipal(), (PasswordToken) regUserWithAlter.getToken());
            client.tableOperations().create(tableName);
            RestartFramework.at("after_table_create_wait_flush").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            rootSecOps.grantTablePermission(regUserWithWrite.getPrincipal(), tableName, TablePermission.WRITE);
            rootSecOps.grantTablePermission(regUserWithAlter.getPrincipal(), tableName, TablePermission.ALTER_TABLE);
            RestartFramework.at("after_grant_permissions_wait_flush").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            tableId = client.tableOperations().tableIdMap().get(tableName);
        }
        ...
        expectPermissionSuccess(op, rootUser);
        op = client -> {
            client.waitForFlush(TraceUtil.traceInfo(), regularUser.toThrift(instanceId), tableId, TextUtil.getByteBuffer(new Text("myrow")), TextUtil.getByteBuffer(new Text("myrow~")), flushId.get(), 1);
            return null;
        };
        expectPermissionDenied(op, regularUser);
        // privileged users can grant themselves permission, but it's not default
        op = client -> {
            client.waitForFlush(TraceUtil.traceInfo(), privilegedUser.toThrift(instanceId), tableId, TextUtil.getByteBuffer(new Text("myrow")), TextUtil.getByteBuffer(new Text("myrow~")), flushId.get(), 1);
            return null;
        };
        expectPermissionDenied(op, privilegedUser);
        op = client -> {
            client.waitForFlush(TraceUtil.traceInfo(), regUserWithWrite.toThrift(instanceId), tableId, TextUtil.getByteBuffer(new Text("myrow")), TextUtil.getByteBuffer(new Text("myrow~")), flushId.get(), 1);
            return null;
        };
        expectPermissionSuccess(op, regUserWithWrite);
        op = client -> {
            client.waitForFlush(TraceUtil.traceInfo(), regUserWithAlter.toThrift(instanceId), tableId, TextUtil.getByteBuffer(new Text("myrow")), TextUtil.getByteBuffer(new Text("myrow~")), flushId.get(), 1);
            return null;
        };
        expectPermissionSuccess(op, regUserWithAlter);
        // root user can because they created the table
        op = client -> {
            client.waitForFlush(TraceUtil.traceInfo(), rootUser.toThrift(instanceId), tableId, TextUtil.getByteBuffer(new Text("myrow")), TextUtil.getByteBuffer(new Text("myrow~")), flushId.get(), 1);
            return null;
        };
        expectPermissionSuccess(op, rootUser);
    }
    ...
    public void testPermissions_modifySystemProperties() throws Exception {
        ...
        op = client -> {
            client.modifySystemProperties(TraceUtil.traceInfo(), regularUser.toThrift(instanceId), new TVersionedProperties(0, Map.of(propKey, "10000")));
            return null;
        };
        expectPermissionDenied(op, regularUser);
        op = client -> {
            client.modifySystemProperties(TraceUtil.traceInfo(), rootUser.toThrift(instanceId), new TVersionedProperties(0, Map.of(propKey, "10000")));
            return null;
        };
        expectPermissionSuccess(op, rootUser);
        op = client -> {
            client.modifySystemProperties(TraceUtil.traceInfo(), privilegedUser.toThrift(instanceId), new TVersionedProperties(1, Map.of(propKey, "10000")));
            return null;
        };
        expectPermissionSuccess(op, privilegedUser);
        try (AccumuloClient client = Accumulo.newClient().from(getClientProps()).build()) {
    ...
    public void shutdownTabletServer() throws Exception {
        ...
        try (AccumuloClient client = Accumulo.newClient().from(getClientProps()).as(rootUser.getPrincipal(), rootUser.getToken()).build()) {
    ...
    public void z99_testPermissions_shutdown() throws Exception {
        ...
        var rootUserBuilder = Accumulo.newClient().from(getClientProps()).as(rootUser.getPrincipal(), rootUser.getToken());
        var privUserBuilder = Accumulo.newClient().from(getClientProps()).as(privilegedUser.getPrincipal(), privilegedUser.getToken());
        try (var rootClient = rootUserBuilder.build();
            var privClient = privUserBuilder.build()) {
    ...
    private static void expectPermissionDenied(ThriftClientTypes.Exec<Void, ManagerClientService.Client> op, Credentials user) {
        AccumuloSecurityException e = assertThrows(AccumuloSecurityException.class, () -> expectPermissionSuccess(op, user));
