...
public class UnusedWALIT_RestartInjected_Randomized_123 extends ConfigurableMacBase {
    ...
    public void test() throws Exception {
        ...
        RestartFramework.at("after_gc_stop").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        // make two tables
        ...
        try (AccumuloClient c = Accumulo.newClient().from(getClientProperties()).build()) {
            ...
            RestartFramework.at("after_table_creates").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            ...
            RestartFramework.at("after_initial_writes").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertEquals(2, getWALCount(context));
            RestartFramework.at("after_first_wal_count").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // roll the logs by pushing data into bigTable
            writeSomeData(c, bigTable, 0, 3000, 0, 1000);
            RestartFramework.at("after_roll_logs").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            assertEquals(3, getWALCount(context));
            RestartFramework.at("after_second_wal_count").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // put some data in the latest log
            writeSomeData(c, lilTable, 1, 10, 0, 10);
            scanSomeData(c, lilTable, 1, 10, 0, 10);
            RestartFramework.at("after_second_writes").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            // bounce the tserver
            ...
            RestartFramework.at("after_metadata_online").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
