...
public class LogicalTimeIT_RestartInjected_Randomized_456 extends AccumuloClusterHarness {

    private static final Logger log = LoggerFactory.getLogger(LogicalTimeIT_RestartInjected.class);
    ...
    private void runMergeTest(AccumuloClient client, String table, String[] splits, String[] inserts, String start, String end, String last, long expected) throws Exception {
        RestartFramework.at("after_final_mutation").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        log.info("table {}", table);
        TreeSet<Text> splitSet = new TreeSet<>();
        for (String split : splits) {
            splitSet.add(new Text(split));
        }
        client.tableOperations().create(table, new NewTableConfiguration().setTimeType(TimeType.LOGICAL).withSplits(splitSet));
        ...
        RestartFramework.at("after_initial_batch_write").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        bw.flush();
        ...
        RestartFramework.at("after_table_create_logical").on(getCluster()).restart("manager").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
        m.put("cf", "cq", "v");
        bw.addMutation(m);
        bw.flush();
