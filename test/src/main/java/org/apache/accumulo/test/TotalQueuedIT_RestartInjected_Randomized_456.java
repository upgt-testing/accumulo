...
public class TotalQueuedIT_RestartInjected_Randomized_456 extends ConfigurableMacBase {
    ...
    public void test() throws Exception {
        try (AccumuloClient c = Accumulo.newClient().from(getClientProperties()).build()) {
            ...
            System.out.printf("Sent %d bytes in %f secs approximately %d syncs (%f syncs per sec)%n", bytesSent, secs, ((long) syncs), syncsPerSec);
            ...
            RestartFramework.at("after_property_change_and_flush").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            sleepUninterruptibly(1, TimeUnit.SECONDS);
            ...
            RestartFramework.at("after_tserver_restart_and_table_online").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
            try (BatchWriter bw = c.createBatchWriter(tableName, cfg)) {
            ...
            RestartFramework.at("after_second_batch_write").on(getCluster()).restart("tablet_server").withIndex(0).withMode(RestartMode.GRACEFUL).execute();
    ...
    private long getSyncs(AccumuloClient c) throws Exception {
        ...
        for (String address : c.instanceOperations().getTabletServers()) {
            TabletClientService.Client client = ThriftUtil.getClient(ThriftClientTypes.TABLET_SERVER, HostAndPort.fromString(address), context);
